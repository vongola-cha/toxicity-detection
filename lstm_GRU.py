...
class GRU(nn.Module):
    def __init__(self):
        ...
        self.embedding_layer = nn.Embedding(num_embeddings=len(vocab), embedding_dim=embed_len)
        self.GRU = nn.GRU(input_size=embed_len, bidirectional=True,hidden_size=hidden_dim, num_layers=n_layers, batch_first=True)
        self.linear = nn.Linear(hidden_dim*2, 1)
...
optimizer = Adam(GRU_predictor.parameters(), lr=learning_rate)
TrainModel(GRU_predictor, loss_fn, optimizer, dataloader, testloader, epoch)
PATH = "GRU.pt"
torch.save(GRU_predictor, PATH)
